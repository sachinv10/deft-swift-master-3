                    ...
                    .observeSingleEvent(of: DataEventType.value) { (pDataSnapshot) in
                        ...
                        if let aDict = pDataSnapshot.value as? Dictionary<String,Any> {
                            if let anIpAddress = aDict["ip"] as? String
                               , let aPort = aDict["port"] as? String
                               , var aUrlComponents = URLComponents(string: "http://" + anIpAddress + ":" + aPort) {
                                aUrlComponents.path = "/api/deft/notification/logs"
                                var aQueryItemArray = Array<URLQueryItem>()
                                aQueryItemArray.append(URLQueryItem(name: "uid", value: Auth.auth().currentUser!.uid))
                                aQueryItemArray.append(URLQueryItem(name: "notificationType", value: pAppNotificationType.rawValue))
